...
import { getStoragedTravels } from './userServices/storagedTravels';
...
const Travels = () => {
    ...
    useEffect(() => {
        const fetchTravels = async () => {
            try {
                const data = await getStoragedTravels(id); 
                setTravels(data);
            } catch (error) {
    ...
    // Función para obtener la imagen según el destino del viaje
        ...
        switch (destino.toLowerCase()) {
            ...
            case 'buenos aires':
                ...
                case 'mendoza':
                    return MendoImage;
                default:
                return DefaultImage; // Imagen genérica cuando no hay imagen específica
        }
    };
    ...
    return (
        <div>
            <h1 className='text-purple-800 text-3xl font-bold'>Viajes archivados</h1>
            <button
                className='w-full md:w-auto py-4 px-12 mt-10 m-2 bg-purple-700 hover:bg-purple-900 text-white font-semibold'
                onClick={goToNewTravel}
            >
                Agregar viaje
            </button>
            <button
                className='w-full md:w-auto py-4 px-10 mt-10 m-2 bg-purple-700 hover:bg-purple-900 text-white font-semibold'
                onClick={goToTravels}
            >
                Ver viajes activos
            </button>
            <div className='mt-6 mb-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
                {travels.map(travel => (
                    <div key={travel._id} className='m-4 text-white bg-purple-600 p-4 rounded-md'>
                        ...
                        {/* Uso de la función getCityImage para obtener la imagen según el destino */}
                        <img
                            src={getCityImage(travel.destino)}
                            alt={`Imagen de ${travel.destino}`}
                            className="w-full h-64 object-cover mb-2 rounded-md my-4"
                        />
