...
import { 
    ...
    getActivityByName, 
    createActivity 
} from "../controllers/activities_controller.js";
...
router.post('/nueva-actividad', upload.single('image'), createActivity);
...
router.put('/', verificarToken, upload.single('image'), async (req, res) => {
    const { _id, nombre, ciudad, informacion_general, direccion, precio, estrellas } = req.body;
    let updatedData = { nombre, ciudad, informacion_general, direccion, precio, estrellas };

    // Si se subió una nueva foto, agregar la URL al objeto updatedData
    if (req.file) {
        updatedData.image = `${req.file.filename}`;
    }

    try {
        const result = await updateActividad(updatedData, _id);
        res.status(200).json({ message: "Actividad actualizada correctamente", data: result });
    } catch (error) {
